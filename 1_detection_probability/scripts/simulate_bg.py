...
"""
Script to run parallel simulations of background.
"""
...
if COMM.rank == 0:
    ...
    N = int(5e5)
    Emin = 1e3  # GeV
    output_file = FILE_STEM + "output/bg_5e5.h5"
    ...
    # Sources
    # Atmospheric neutrinos and diffuse astrophysical flux
    broken_power_law = BrokenPowerLawFlux(
        2.0e-12, 2.5e3, 3.1, 3.7, lower_energy=Emin, upper_energy=1e6
    )
    atmospheric = DiffuseSource(flux_model=broken_power_law)
